...
void mt7921_mac_add_txs(struct mt792x_dev *dev, void *data)
{
        ...
        {
                static atomic_t sl26_txs;
                int sln = atomic_inc_return(&sl26_txs);

                if (sln <= 16 || sln % 500 == 0)
                        dev_info(dev->mt76.dev,
                                 "sl0026a: TXS#%d fmt=%u wcid=%u pid=%u\n", sln,
...
static void mt7921_mac_tx_free(struct mt792x_dev *dev, void *data, int len)
{
        ...
        {
                static atomic_t sl26_txf;
                int sln = atomic_inc_return(&sl26_txf);

                if (sln <= 16 || sln % 500 == 0)
                        dev_info(dev->mt76.dev,
                                 "sl0026a: TXFREE#%d msdu=%u\n", sln, count);
        }
...
void mt7921_mac_reset_work(struct work_struct *work)
{
        ...
        /* 0024: reset drops fw-RDD/vif references (firmware lost all
         * state); the post-reset mt7921_dfs_offload_update() below
         * re-arms RDD if the AP still operates on a DFS channel. */
