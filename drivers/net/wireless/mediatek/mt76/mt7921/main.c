                    ...
                    struct ieee80211_sband_iftype_data *data)
{
        ...
        for (i = 0; i < NUM_NL80211_IFTYPES; i++) {
                ...
                case NL80211_IFTYPE_AP:
                        ...
                        /* TWT responder capability — TASK-007 */
                        he_cap_elem->mac_cap_info[0] |=
...
static void __mt7921_dfs_rdd_eval(struct mt792x_dev *dev,
                                  const struct cfg80211_chan_def *chandef,
                                  struct ieee80211_vif *vif);
static void mt7921_dfs_rdd_disarm(struct mt792x_dev *dev);
                                    ...
                                    struct ieee80211_vif *vif)
{
        ...
        /* 0024: drop the DFS radar-report vif reference before it dies. */
        mt792x_mutex_acquire(dev);
                                    ...
                                    u64 changed)
{
        ...
        /* 0023/0025: beacon up/down re-evaluates the DFS RDD state
         * from the vif's chanctx (the AP operating channel). */
        if (changed & BSS_CHANGED_BEACON_ENABLED)
                __mt7921_dfs_rdd_eval(dev, NULL, vif);
                   ...
                   struct ieee80211_chanctx_conf *ctx)
{
        ...
        /* 0025: new chanctx (CAC start / CSA target) - evaluate RDD. */
        mt792x_mutex_acquire(dev);
        __mt7921_dfs_rdd_eval(dev, &ctx->def, NULL);
        mt792x_mutex_release(dev);
                      ...
                      u32 changed)
{
        ...
        /* 0025: chanctx def changed (CSA/width) - re-evaluate RDD. */
        __mt7921_dfs_rdd_eval(phy->dev, &ctx->def, vif);
        mt792x_mutex_release(phy->dev);
}
...
/* ============== 0024: DFS radar-monitoring engine ==============
 * 6.18 contract (verified against v6.18 kernel source):
 *  - nl80211_start_radar_detection() handles CAC: it computes the CAC
 *    time, calls mac80211, which applies the channel (link_use_channel
 *    -> chanctx -> .config(CHANGE_CHANNEL)) and arms its own per-link
 *    dfs_cac_timer_work; cfg80211 tracks wdev->links[].cac_started.
 *    Manual CAC start is REJECTED while DFS_OFFLOAD is advertised,
 *    so this driver must NOT advertise it.
 *  - The driver's DFS obligations: enable firmware RDD (radar pulse
 *    detection) while on a radar channel, and report radar hits via
 *    ieee80211_radar_detected() so mac80211 + hostapd CSA-vacate.
 *    CAC bookkeeping and NL80211_RADAR_CAC_* events belong to the
 *    kernel, not the driver.
 *
 * Triggers (idempotent, dev->mt76.mutex held):
 *   - .config(IEEE80211_CONF_CHANGE_CHANNEL) (chanctx apply incl. CAC)
 *   - bss_info_changed(BSS_CHANGED_BEACON_ENABLED)
 *   - post-chip-reset re-evaluation (mt7921_mac_reset_work tail)
 *   - vif removal clears the vif ref (mt7921_remove_interface)
 */
...
/* Disarm firmware RDD and drop the vif reference (channel change, AP
 * down, vif removal, chip reset). Process context, may send an MCU
 * command. Caller holds dev->mt76.mutex. */
static void mt7921_dfs_rdd_disarm(struct mt792x_dev *dev)
{
...
{
        struct mt7921_dfs_state *dfs = &dev->phy.dfs_state;
        const struct cfg80211_chan_def *def = chandef;
        struct ieee80211_channel *chan;

        /* No explicit chandef: fall back to the AP vif's operating
         * chanctx (beacon up / post-reset re-evaluation). */
        if (!def) {
                if (!vif)
        ...
        if (!def || !def->chan)
                return; /* no authoritative AP channel: keep state */

        chan = def->chan;

        if (!(chan->flags & IEEE80211_CHAN_RADAR)) {
                /* left DFS: end radar monitoring */
                if (dfs->rdd_enabled)
                        mt7921_dfs_rdd_disarm(dev);
                return;
        }
        ...
        if (chan->dfs_state == NL80211_DFS_UNAVAILABLE) {
                if (dfs->rdd_enabled) {
                        dev_dbg(dev->mt76.dev,
                                "sl0023: DFS ch %d in NOP - radar monitoring off\n",
                                chan->hw_value);
                        mt7921_dfs_rdd_disarm(dev);
                }
                return;
        ...
        /* Radar channel, CAC pending (USABLE) or satisfied (AVAILABLE):
         * firmware must watch for radar pulses the whole time. */
        if (!dfs->rdd_enabled) {
                int ret = mt7921_mcu_rdd_set(dev, true);

                if (ret) {
                        dev_err(dev->mt76.dev,
                                "sl0023: firmware RDD enable failed (%d) - radar detection inactive\n",
                                ret);
                        return;
                }
                dfs->rdd_enabled = true;
                dev_info(dev->mt76.dev,
                         "sl0023: DFS ch %d - fw RDD radar monitoring armed (CAC belongs to mac80211)\n",
                         chan->hw_value);
        }

        /* Keep the AP vif reference for the radar-report path
...
{
        mt792x_mutex_acquire(dev);
        __mt7921_dfs_rdd_eval(dev, NULL, NULL);
        mt792x_mutex_release(dev);
}
                                        ...
                                        struct sk_buff *skb)
{
         ...
         */
        if (dfs->cac_vif) {
                mt792x_radar_detected(dev->mt76.phy.hw, dfs->cac_vif);
                dfs->cac_vif = NULL;
        }
}
