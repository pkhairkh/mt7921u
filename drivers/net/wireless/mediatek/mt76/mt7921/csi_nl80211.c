                             ...
                             const void *data, int data_len)
{
        ...
        struct mt7921_csi_data local_entry;
        struct mt7921_csi_data *entry = &local_entry;
        struct sk_buff *msg;
        struct nlattr *attrs[MT7921_NL_ATTR_CSI_MAX + 1];
        unsigned long flags;
        int ret;
        ...
        /* Use the CSI ring's spinlock to match the producer (mcu_csi_event).
         * Copy out the entry under spinlock, then release before building
         * the netlink message (which may sleep — cannot hold spinlock).
         */
        ...
        tail = csi->tail;
        memcpy(&local_entry, &csi->buffer[tail], sizeof(local_entry));

        /* Advance tail under spinlock */
