...
struct mt7921_dfs_state {
        ...
        /* 0023/0024: DFS radar-monitoring state (fw RDD + report vif) */
        bool rdd_enabled;
        u32 cac_center_freq;
        struct cfg80211_chan_def cac_chandef;
};
#endif
...
struct mt792x_sta {
        ...
        /* TWT (Target Wake Time) state — TASK-007 */
        struct {
                u8 flowid_mask;
                struct mt7921_twt_flow flow[MT7921_MAX_STA_TWT_AGRT];
        } twt;
};
...
struct mt792x_dev {
        ...
        /* TWT (Target Wake Time) device state — TASK-007 */
        struct {
                u16 table_mask;
                u8 n_agrt;
