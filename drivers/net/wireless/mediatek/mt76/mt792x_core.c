...
int mt792x_init_wiphy(struct ieee80211_hw *hw)
{
        ...
        /* 0024: classic DFS contract. On 6.18, mac80211 itself runs
         * the CAC state machine (ieee80211_start_radar_detection ->
         * link_use_channel + per-link dfs_cac_timer_work), and
         * nl80211 REJECTS manual CAC start (-EOPNOTSUPP) while
         * NL80211_EXT_FEATURE_DFS_OFFLOAD is advertised. Advertising
         * offload would push hostapd into autonomous-wait mode whose
         * trigger (AP preset_chandef) never reaches the driver. The
         * driver's DFS role: arm firmware RDD monitoring on radar
         * channels, report hits via ieee80211_radar_detected(). */
