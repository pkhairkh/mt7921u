...
/* sl0026a (mt76-usb): TX endpoint lifecycle dump */
extern void sl0026a_tx_dump(struct mt76_dev *dev);
...
void mt792x_mac_work(struct work_struct *work)
{
        ...
        /* sl0026a: read-only TX endpoint dump, once per second */
        {
                static atomic_t sl26_tick;

                if (mt76_is_usb(mphy->dev) &&
                    atomic_inc_return(&sl26_tick) % 2400 == 0)
                        sl0026a_tx_dump(mphy->dev);
        }
...
mt792x_phy_get_nf(struct mt792x_phy *phy, int idx)
{
        ...
        /* Read per-antenna noise floor histogram from the IRPI block.
         * The connac2 IRPI base is at WF_IRPI(0x6000 + NSS*16) for
         * the primary phy. Each NSS has 11 histogram bins (4 bytes each)
         * weighted by nf_power[] to compute the average noise floor.
         *
         * Register layout (per connac2/MT7921):
         *   IRPI_BASE + 0x6000 + (nss << 16) + (bin << 2)
         *
         * Each bin counts the number of frames received at that power
         * level. The weighted average gives the noise floor in dBm.
         */
        for (nss = 0; nss < hweight8(phy->mt76->chainmask); nss++) {
                /* IRPI register offset: connac2 uses 0x83000000 base,
                 * with per-NSS histogram at +0x6000 + (nss << 16).
                 * Each bin is 4 bytes apart.
                 */
                u32 reg = MT_WF_IRPI_BASE + 0x6000 + (nss << 16);
